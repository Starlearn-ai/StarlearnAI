...
const Signup = () => {
  ...
  // Fix 1: Added empty dependency array so useEffect runs only once on mount.
  useEffect(() => {
  ...
  // Updated sendEmail function to accept 'mName'
  async function sendEmail(mEmail: string, mName: string) {
    try {
      ...
      await axios.post(postURL, dataToSend).then(res => {
        // We can still redirect here, as the email sending is somewhat independent
        redirectHome();
      }).catch(error => {
        console.error("Error sending welcome email:", error);
        // Decide if you want to redirect home even if email fails.
        // For now, keeping your original behavior of redirecting.
        redirectHome();
      });

    } catch (error) {
      console.error("Error in sendEmail function:", error);
      redirectHome(); // Also redirect on outer catch
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Fix 2: Calling sendEmail only once, and passing the name
        await sendEmail(email, name);
        // redirectHome will be called inside sendEmail's .then() or .catch()
      } else {
        setError(response.data.message);
        setIsLoading(false); // Make sure to set isLoading to false on API error
      }
      ...
      setIsLoading(false); // Ensure isLoading is always false at the end of the submission attempt
    }
  };
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4 py-12">
      <div className="w-full max-w-md space-y-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-muted-foreground">Try {appName} for free</p>
        </div>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <GoogleLogin
              ...
              type='standard'
              width={400}
              onSuccess={async (credentialResponse: any) => { // Type 'any' for jwtDecode compatibility
                try {
                  setIsLoading(true);
                  const decoded: any = jwtDecode(credentialResponse.credential); // Type 'any' for jwtDecode compatibility
                  const email = decoded.email;
                  const name = decoded.name;
                  const postURL = serverURL + '/api/social';

                  // Recommendation: In a real application, send the actual credentialResponse.credential
                  // to your backend for verification with Google's API. For this example,
                  // we're proceeding with sending email and name directly.
                  ...
                  if (response.data.success) {
                    ...
                    // Fix 3: Calling sendEmail for social login, passing the name
              ...
              onError={() => {
                setIsLoading(false);
                setError('Google login failed. Please try again.'); // More specific error
              }}
            />
            ...
            <FacebookLogin
              ...
              onFail={(error: any) => { // Type 'any' for error compatibility
                console.error("Facebook login failed:", error);
                setIsLoading(false);
                setError('Facebook login failed. Please try again.'); // More specific error
              }}
              onProfileSuccess={async (response: any) => { // Type 'any' for response compatibility
                try {
                  setIsLoading(true);
                  ...
                  // Recommendation: In a real application, send the actual access token
                  // to your backend for verification with Facebook's Graph API. For this example,
                  // we're proceeding with sending email and name directly.
                  const apiResponse = await axios.post(postURL, { email, name }); // Renamed 'response' to 'apiResponse' to avoid conflict
                  ...
                  if (apiResponse.data.success) {
                    ...
                    // Fix 4: Calling sendEmail for social login, passing the name
                    await sendEmail(email, name);
                  } else {
